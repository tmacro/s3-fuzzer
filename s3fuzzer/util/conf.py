...
from s3fuzzer import __version__ as pkg_version

from .arg import parse_args
from .log import parse_loglvl

# from s3fuzzer import __name__ as pkg_name
pkg_name = 's3fuzz'
...
def get_from_env(key, default = None, type = None):
...
MODULE_CONFIG = {
	...
	'base_filename': 'config', # Searches `pkgname` `.pkgname` `.pkgname.<ext>` `<pkgname.<ext>`
...
@loader()
def cli_loader(config):
	v = parse_args(pkg_name)
	return v
...
def load_config(modconf):
	...
	for loader in MODULE_CONFIG['additional_loaders']:
		conf = recurse_update(conf, loader(create_namespace(conf)))
